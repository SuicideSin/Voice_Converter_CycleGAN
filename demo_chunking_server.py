...
class Converter():
    ...
    def convert(self, wav, conversion_direction='A2B'):
        stereo_pcm_data = self.convert_to_pcm_data(wav, conversion_direction=conversion_direction)
...
class ApiHandler():
    def on_post(self, request, response):
        ...
        for chunk in chunks:
            ...
            pcm_data = converter.convert_to_pcm_data(chunk, conversion_direction='A2B')
            chunk_results.append(pcm_data)

        print(">>> Chunk Results:")
        print(len(chunk_results))
        print(len(chunk_results[0]))
        print(type(chunk_results[0]))

        concat_chunks = np.copy(chunk_results[0])

        for chunk in chunk_results[1:]:
            print("Concat_chunks.shape (before) = {}".format(concat_chunks.shape))
            print("Chunk.shape = {}".format(chunk.shape))
            concat_chunks = np.concatenate((concat_chunks, chunk))
            print("Concat_chunks.shape (after) = {}".format(concat_chunks.shape))

        print("! Final concat_chunks.shape = {}".format(concat_chunks.shape))
        print("! (Original algo) downsampled.shape = {}".format(downsampled.shape))

        # Evaluate the model
        #results = converter.convert_to_wav_buf(downsampled, conversion_direction = 'A2B')
        results = converter.convert_pcm_to_wav(concat_chunks)
        print(">>> Model results:")
        print(results)
